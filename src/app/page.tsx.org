...
// 1. The List of Examples
  ...
  const handleExampleClick = (text: string) => {
    ...
    handleGenerate(text); // Trigger the AI immediately
  };
...
const handleGenerate = async (overrideInput?: string) => {
    const textToUse = overrideInput || input; // Use override if provided
    if (!textToUse) return;
    setLoading(true);
    setIsEditing(false);
    setResult(null); 
...
// Trigger Success Modal with count 1
        setSuccessCount(1);
	setSuccessCode(saved.code); // <--- ADD THIS
        setShowSuccess(true);

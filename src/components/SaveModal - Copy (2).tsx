...
import { X, Upload, FileSpreadsheet, Loader2, Save, User, Trash2, FileText, CheckCircle2, Crown, Lock } from "lucide-react";
...
export function SaveModal({ isOpen, onClose, onSaveSingle, onSaveBulk, currentName }: SaveModalProps) {
  ...
  // Stores the rules returned from the server (limit, tier, canUploadCsv)
  ...
  useEffect(() => {
    if (isOpen) {
        // Fetch the dynamic rules when modal opens
        getUserLimits().then(setUserRules);
        
        setManualText(currentName);
        setCsvFile(null);
        setPreviewNames([]);
        setMode('manual'); // Reset to manual tab
    }
  }, [isOpen, currentName]);

  if (!isOpen) return null;

  // --- DERIVED VALUES (The "Brain" of the UI) ---
  // If rules haven't loaded yet, default to Guest settings (Limit 5, No CSV)
  ...
  const checkLimits = (count: number) => {
      // Logic: If user exceeds their specific batch limit
      if (count > limit) {
          setPricingState({ 
            isOpen: true, 
  ...
  const handleStandardProcess = async () => {
    ...
    // For CSV, we assume if they can access this tab, they are allowed
    setIsProcessing(true);
  ...
  const handlePartialProcess = async () => {
    ...
    // Process only up to the allowed limit
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between p-4 border-b border-slate-100 shrink-0">
              <h3 className="font-bold text-slate-900 flex items-center gap-2">
                Save Certificate
                {/* Optional: Show Badge */}
                {userRules?.tier === 'verified' && <span className="bg-blue-100 text-blue-600 text-[10px] px-2 py-0.5 rounded-full uppercase">Verified</span>}
              </h3>
              <button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>

            {/* === TABS WITH LOCK LOGIC === */}
            <div className="flex border-b border-slate-100 shrink-0">
              <button 
                onClick={() => setMode('manual')} 
                className={`flex-1 py-3 text-sm font-medium transition-colors ${mode === 'manual' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-slate-500 hover:bg-slate-50'}`}
              >
                Manual Entry
              </button>
              ...
              <button 
                onClick={() => {
                    // If not allowed, open pricing instead of switching tabs
                    if (canUploadCsv) {
                        setMode('csv');
                    } else {
                ...
                className={`flex-1 py-3 text-sm font-medium transition-colors flex items-center justify-center gap-2
                    ${mode === 'csv' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-slate-500 hover:bg-slate-50'}
                    ${!canUploadCsv ? 'opacity-60 cursor-not-allowed bg-slate-50' : ''}
                `}
              >
                  ...
                  <div>
                    ...
                    <div className="flex justify-between items-start">
                        <p className="text-[10px] text-slate-400">Enter names separated by commas or new lines.</p>
                         {/* Only show "Sign in" prompt if they are a guest */}
                        {userRules?.tier === 'guest' && (
                            <p className="text-[10px] text-blue-500 cursor-pointer hover:underline" onClick={() => router.push('/login')}>
